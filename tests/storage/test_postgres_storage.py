...
class TestPostgresStorage(unittest.TestCase):
    ...
    @patch('storage.postgres_storage.create_engine')
    @patch('storage.postgres_storage.sessionmaker')
    def setUp(self, mock_sessionmaker, mock_create_engine):
        ...
        self.storage = PostgresStorage(connection_string="postgresql://user:pass@localhost/testdb")
    ...
    def test_load_assistant(self):
        ...
        assistant = AssistantModel(id="assistant_1", name="Test Assistant", model="GPT-3", data=json.dumps({"key": "value"}))
        mock_session.query.return_value.filter_by.return_value.first.return_value = assistant
    ...
    def test_load_thread(self):
        ...
        # Mock the return value of the query
        thread = ThreadModel(id="thread_1", assistant_id="assistant_1", data=json.dumps({"key": "value"}))
        mock_session.query.return_value.filter_by.return_value.first.return_value = thread
